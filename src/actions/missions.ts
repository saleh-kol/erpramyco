...
export async function getMissions(status: string = 'All') {
  ...
  if (status && status !== 'All') {
    // حرف اول را بزرگ می‌کنیم تا با Enum دیتابیس مطابقت داشته باشد (مثلاً pending تبدیل به Pending شود)
...
export async function updateMissionAction(formData: FormData): Promise<void> {
  ...
  if (session) {
    const user = await verifySession(session); // رمزگشایی صحیح توکن
    if (user) {
