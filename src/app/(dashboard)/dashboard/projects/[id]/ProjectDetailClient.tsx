...
import {
  ArrowRight, Save, CheckCircle, Trash2, AlertTriangle, X, Briefcase, Users
} from "lucide-react";
...
export default function ProjectDetailClient({ data }: { data: any }) {
  ...
  const [startDateVal, setStartDateVal] = useState<any>(data.Start_Date ? new Date(data.Start_Date) : null);
  const [endDateVal, setEndDateVal] = useState<any>(data.End_Date ? new Date(data.End_Date) : null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setToast("تغییرات پروژه با موفقیت ذخیره شد");
  ...
  const handleDelete = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result?.success) {
      ...
      setToast("پروژه با موفقیت حذف شد"); 
      setTimeout(() => {
        router.push("/dashboard/projects"); 
      }, 2000);
    } else {
      // نمایش دقیق خطای دیتابیس
      setShowDeleteModal(false);
      setToast(result?.error || "خطا در حذف پروژه");
    ...
    <div style={{ backgroundColor: "#f1f5f9", minHeight: "100vh", padding: "24px" }}>
      ...
      <div style={{ backgroundColor: "white", borderRadius: "20px", padding: "20px 24px", marginBottom: "20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <button onClick={() => router.push("/dashboard/projects")} style={{ border: "1px solid #e2e8f0", borderRadius: "12px", padding: "10px", cursor: "pointer", backgroundColor: "white" }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: "13px", color: "#64748b" }}>{data.Project_Code}</p>
          </div>
        </div>
        <button onClick={() => setShowDeleteModal(true)} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 20px", borderRadius: "12px", border: "1px solid #fecaca", cursor: "pointer", fontWeight: "bold", backgroundColor: "#fef2f2", color: "#991b1b" }}>
          <Trash2 style={{ width: "18px", height: "18px" }} /> حذف پروژه
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "20px" }}>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "20px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>کد پروژه</label><input name="projectCode" defaultValue={data.Project_Code} type="text" className="erp-input" /></div>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>نام پروژه</label><input name="projectName" defaultValue={data.Project_Name} type="text" className="erp-input" /></div>
          </div>

          <div style={{ marginBottom: "16px" }}>
            <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>شرح پروژه</label>
            <textarea name="description" rows={4} defaultValue={data.Description || ""} className="erp-input" style={{ resize: "vertical" }}></textarea>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>تاریخ شروع</label>{isMounted ? <DatePicker value={startDateVal} calendar={persian} locale={persian_fa} calendarPosition="bottom-right" onChange={setStartDateVal} format="YYYY/MM/DD" style={dateInputStyle} /> : <input type="text" className="erp-input" disabled />}</div>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>تاریخ پایان</label>{isMounted ? <DatePicker value={endDateVal} calendar={persian} locale={persian_fa} calendarPosition="bottom-right" onChange={setEndDateVal} format="YYYY/MM/DD" style={dateInputStyle} /> : <input type="text" className="erp-input" disabled />}</div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>بودجه (ریال)</label><input name="budget" defaultValue={data.Budget || 0} type="number" min="0" className="erp-input" /></div>
            <div><label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>وضعیت پروژه</label>
              <select name="status" defaultValue={data.Status} className="erp-input">
        ...
        {/* ستون چپ: امتیاز مدیرعامل و تیم پروژه */}
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          
          {/* باکس امتیاز و نظر مدیرعامل */}
          <div style={{ backgroundColor: "white", borderRadius: "20px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            <h2 style={{ margin: "0 0 20px 0", fontSize: "16px", fontWeight: "bold", color: "#0f172a", display: "flex", alignItems: "center", gap: "8px" }}>
              <div style={{ width: "4px", height: "20px", backgroundColor: "#ed6e2b", borderRadius: "2px" }}></div>
              ارزیابی مدیرعامل
            ...
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>امتیاز پروژه (۰ تا ۱۰۰)</label>
          ...
          <div style={{ backgroundColor: "white", borderRadius: "20px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {data.PR_Project_Assignments?.map((pa: any) => (
                <div key={pa.Assignment_ID} style={{ display: "flex", alignItems: "center", gap: "10px", backgroundColor: "#f8fafc", padding: "12px", borderRadius: "10px", border: "1px solid #f1f5f9" }}>
                  <div style={{ background: "#fff7ed", padding: "8px", borderRadius: "10px" }}><Users style={{ width: "16px", height: "16px", color: "#ed6e2b" }} /></div>
                  <div>
                    <p style={{ margin: "0 0 4px 0", fontWeight: "bold", fontSize: "14px", color: "#0f172a" }}>{pa.Personnel?.Full_Name}</p>
                    <p style={{ margin: 0, fontSize: "12px", color: "#64748b" }}>{translateRole(pa.Personnel?.Role)}</p>
                  </div>
                </div>
              ))}
              {data.PR_Project_Assignments?.length === 0 && <p style={{ fontSize: "14px", color: "#94a3b8", textAlign: "center" }}>عضوی برای این پروژه تعریف نشده است</p>}
            </div>
          </div>
        </div>
